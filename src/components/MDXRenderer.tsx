...
export const MDXRenderer: React.FC<MDXRendererProps> = ({ source, courseId, articleSlug }) => {
  // TryIt 索引計數器
  let tryItIndex = 0;
  ...
  const components = {
    // 自定義組件 - 注入課程資訊和索引
    TryIt: (props: any) => {
      const currentIndex = ++tryItIndex;
      return <TryIt {...props} courseId={courseId} articleSlug={articleSlug} index={currentIndex} />;
    },
    Pitfalls,
    Why,
    Goals,
    Boss: (props: any) => <Boss {...props} courseId={courseId} articleSlug={articleSlug} />,
    ...
    code: ({ children }: { children: React.ReactNode }) => (
      <code className="bg-stone-700 text-orange-400 px-2 py-1 rounded text-sm md:text-base font-mono">
        {children}
      </code>
    ),
    pre: ({ children }: { children: React.ReactNode }) => (
      <pre className="bg-stone-800 text-stone-300 p-6 rounded-lg overflow-x-auto mb-6 border border-stone-700 min-h-[120px] leading-relaxed text-sm md:text-base">
        {children}
      </pre>
    ),
